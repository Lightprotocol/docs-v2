...
To optimize instruction data, we reference tree accounts by u8 indices instead of their 32-byte pubkeys.

The SDK provides the `PackedAccounts` helper:
* Each account is assigned a sequential u8 index (0, 1, 2...). 
* Duplicate pubkeys return the cached index.
* To build the instruction accounts are concatenated in order.

Programs use these indices to retrieve accounts from the instruction's accounts array.
* Anchor programs store packed accounts in `remainingAccounts`
* Native programs can append packed accounts to the instructions account metas

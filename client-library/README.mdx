...
* Starts a mock RPC instance that parses events and builds Merkle trees.
...
<Note>
Compressed account data must be passed as instruction data because only the Merkle root hash is stored in a Solana account.
...
<Tip>
For instructions other than create, **use the state tree of the existing compressed account** as output state tree.
* **Best practice**: minimize the number of different trees per transaction to keep instruction data light.

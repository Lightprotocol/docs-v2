...
Transactions with compressed accounts reference in their instruction
1. a number of [Light System accounts](https://www.zkcompression.com/resources/addresses-and-urls#system-accounts) for proof verification and CPI calls to update state and address trees
2. a number of Merkle tree accounts

To optimize instruction data, we pack accounts into an array with `PackedAccounts`:
...
This instance contains three empty account sections with subsequent indices.
...
// 3. Pack tree accounts from proof to get indices for instruction data
// Returns index 0, adds tree to accounts array
const addressMerkleTreePubkeyIndex 
  = packedAccounts.insertOrGet(addressTree);
// Returns index 1, adds queue to accounts array  
...
* V2 combines the state tree and nullifier queue into a single account for batched Merkle tree updates.
...
<Tabs>
<Tab title="Typescript">
```typescript create.ts expandable wrap
...
<Tab title="Rust">
```rust create.rs expandable wrap

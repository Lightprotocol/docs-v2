...
Depending on your instruction, your client must fetch metadata of one or multiple Merkle trees.

* **All instructions** require metadata for a **state tree** to store or reference the compressed account hash.
* **Only** when you **create** a compressed account, you additionally fetch an **address tree** to derive and store the account address.
...
<Tab title="Update/Close/Reinitialize/burn">
...
The `PackedAccounts` helper optimizes instruction data by allowing you to reference accounts by u8 indices instead of their 32-byte pubkeys. Programs retrieve accounts via these indices.

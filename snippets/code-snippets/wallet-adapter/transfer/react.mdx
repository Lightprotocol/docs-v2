...
export function useTransfer() {
  ...
  const transfer = async (args: TransferArgs): Promise<string> => {
    ...
    try {
      ...
      await getOrCreateAtaInterface(rpc, owner, mintPubkey, recipient);
      const destination = getAssociatedTokenAddressInterface(mintPubkey, recipient);

      const instructions: TransactionInstruction[][] =
        await createTransferInterfaceInstructions(
          rpc, owner, mintPubkey, tokenAmount, owner, destination,
        );

...
async fn test_create_ata() {
    ...
    let program_id = light_token_macro_create_ata::ID;
    let mut config =
        ProgramTestConfig::new_v2(true, Some(vec![("light_token_macro_create_ata", program_id)]));
    ...
    let instruction_data = light_token_macro_create_ata::instruction::CreateAta {
        params: CreateAtaParams {

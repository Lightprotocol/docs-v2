...
use shared::{
    ...
    create_test_rpc, get_authority_pda, setup_mint_with_pda_authority,
    setup_mint_with_tokens, create_ata,
};
...
async fn mint_to_checked_cpi() {
    ...
    let (mint, atas) = setup_mint_with_tokens(
        &mut rpc,
        &payer,
        payer.pubkey(),
        None,
...
async fn mint_to_checked_signed_cpi() {
    ...
    let mint = setup_mint_with_pda_authority(&mut rpc, &payer, pda_authority, 9).await;

    let recipient = Keypair::new();
    let recipient_ata = create_ata(&mut rpc, &payer, recipient.pubkey(), mint).await;

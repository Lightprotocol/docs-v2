...
- Privy Node starter: https://github.com/privy-io/examples/tree/main/privy-node-starter
- Working example: https://github.com/Lightprotocol/examples-zk-compression/tree/main/privy/nodejs-privy-compressed
- Compressed Token Guide for Privy Nodejs: https://zkcompression.com/compressed-tokens/for-privy 
...
See Code Reference below in # for implementation, all signed with Privy (#6)

1. **Setup** → #1: Connect to ZK Compression RPC (Helius, Triton)
2. **Get Balance** → #2: Fetch SPL and compressed token balances
3. **Transfer** → #3: Send compressed tokens to another recipient, signed with Privy SDK
4. **Compress** → #4: Convert SPL to compressed tokens and send to a recipient in one instruction
5. **Decompress** → #5: Convert compressed tokens back to SPL tokens for offramps
6. **Tx History** → #7: Fetch compressed token transaction history for an owner

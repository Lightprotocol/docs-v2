...
in 0.17.x onward, V2 is a recommended default feature.
...
The `light-client` crate handles v1/v2 automatically. The `get_validity_proof` method routes to the v2 endpoint when the v2 feature is enabled (default):
...
let rpc = LightClient::new(
    LightClientConfig::new("https://mainnet.helius-rpc.com/?api-key=YOUR_KEY")
).await?;

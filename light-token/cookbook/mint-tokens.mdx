...
The `mintToInterface` function mints tokens to light-token accounts in a single call.

<Info>
Find the source code [here](https://github.com/Lightprotocol/light-protocol/blob/main/js/compressed-token/tests/e2e/mint-to-interface.test.ts).
</Info>

<Note>
The function auto-detects the token program (SPL, Token-2022, or CToken) from the mint address.
</Note>
...
async function main() {
    ...
    const destination = getAssociatedCTokenAddress(recipient.publicKey, mint);

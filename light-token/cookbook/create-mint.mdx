...
The `createMintInterface` function creates a light-mint with token metadata in a single call.

<Info>
Find the source code [here](https://github.com/Lightprotocol/light-protocol/blob/main/js/compressed-token/tests/e2e/create-mint-interface.test.ts).
</Info>

<Note>
You can pass `TOKEN_PROGRAM_ID` or `TOKEN_2022_PROGRAM_ID` as the `programId` to create SPL or Token-2022 mints for interoperability testing.
</Note>
...
async function main() {
    ...
    // 2. Create light-mint with metadata
    const mintSigner = Keypair.generate();

    const { mint, transactionSignature } = await createMintInterface(
        rpc,
        payer,
        payer,  // mintAuthority (must be Signer for c-token)
        null,   // freezeAuthority
        9,      // decimals
        mintSigner,
        undefined,
        undefined,  // programId: defaults to CTOKEN_PROGRAM_ID. Use TOKEN_PROGRAM_ID for SPL mints.
...
async function main() {
    ...
    // 3. Get tree infos
    // the address Merkle tree stores the address for the mint account
    // the state Merkle tree stores the mint account
    ...
    // 4. Get validity proof for address creation
    // proofs the address for the mint account does not exist yet
    ...
    console.log('Light-mint created:', mintPda.toBase58());

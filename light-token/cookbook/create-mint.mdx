...
description: Create a Light mint, an SPL mint, or a Token 2022 mint with an interface PDA, or add an interface PDA to an existing mint.
...
Pass `TOKEN_PROGRAM_ID` to create a standard SPL mint with an interface PDA in one transaction. This enables compression for the mint.
...
Pass `TOKEN_2022_PROGRAM_ID` to create a Token-2022 mint with an interface PDA in one transaction. This enables compression for the mint.
...
Register an interface PDA for an existing SPL mint to enable compression. No mint authority required.

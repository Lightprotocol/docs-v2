...
<Note>
Pass `TOKEN_PROGRAM_ID` or `TOKEN_2022_PROGRAM_ID` as `programId` to create SPL or Token-2022 ATAs.
</Note>
...
import { createAtaInterface, createMintInterface } from '@lightprotocol/compressed-token';
...
async function main() {
    ...
    // 2. Create a light-mint first (or use existing mint)
    ...
    const ataAddress = await createAtaInterface(
        rpc,
        payer,
        mint,
        owner.publicKey,
    ...
    console.log('Light-ATA created:', ataAddress.toBase58());
}
...
async function main() {
  ...
  const rpc = createRpc(`https://devnet.helius-rpc.com?api-key=${apiKey}`);
  ...
  // 3. Create a mint first (or use existing mint)
  const mint = await createMint(rpc, payer);

  // 4. Define owner and derive ATA address
  const owner = Keypair.generate();
...
// Helper: create a mint (see create-mint example for details)
async function createMint(rpc: ReturnType<typeof createRpc>, payer: Keypair): Promise<PublicKey> {
  const mintSigner = Keypair.generate();
  ...
  const ix = createMintInstruction(
    ...
    addressTreeInfo,
    stateTreeInfo,
    createTokenMetadata("Test Token", "TEST", "https://example.com/metadata.json")
  );
  ...
  await sendAndConfirmTx(rpc, tx, { skipPreflight: true });

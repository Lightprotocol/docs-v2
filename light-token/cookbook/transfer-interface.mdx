...
The `transferInterface` function transfers tokens between light-token accounts in a single call.

<Info>
Find the source code [here](https://github.com/Lightprotocol/light-protocol/blob/main/js/compressed-token/tests/e2e/transfer-interface.test.ts).
</Info>

<Note>
The function auto-loads compressed tokens to the sender's hot balance if needed. Destination must already exist.
</Note>
...
<Step>
### Transfer Between Light-Token Accounts
...
async function main() {
    ...
    // 4. Create recipient's ATA (destination must exist)
...
async function main() {
    ...
    const ix = createCTokenTransferInstruction(
        ...
        amount,
        payer.publicKey,    // payer (for rent top-up)
    );

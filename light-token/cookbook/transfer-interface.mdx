...
_ **SPL → light-token**: SPL tokens are locked by the light token program in the PDA, light-tokens are minted to light-token accounts
_ **light-token → SPL**: light-tokens are burned, SPL tokens transferred to SPL token accounts
...
<Tabs>
  <Tab title="invoke (External signer)">```rust transfer.invoke()?; ```</Tab>
  <Tab title="invoke_signed (PDA is signer)">
    ```rust let authority_seeds: &[&[u8]] = &[TRANSFER_INTERFACE_AUTHORITY_SEED,

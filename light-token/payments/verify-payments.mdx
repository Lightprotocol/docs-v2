...
Returns the Light Token ATA balance (hot balance).
...
console.log(account.parsed.amount); // Light Token ATA balance
```
...
To include compressed (cold) and optionally SPL/T22 balances, call `loadAta` first to unify everything into the Light Token ATA, then read the balance.
...
// Unify compressed (cold) balances into the Light Token ATA
await loadAta(rpc, ata, payer, mint, payer);

const account = await getAtaInterface(rpc, ata, owner, mint);
console.log(account.parsed.amount); // Unified balance
```

<Info>
See the [Load Associated Token Account](/light-token/cookbook/load-ata) guide for details on `loadAta`, including wrapping SPL and Token 2022 balances.
</Info>

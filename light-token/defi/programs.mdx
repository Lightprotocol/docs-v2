...
`LightProgram` trait in your program's SDK crate.
...
async fn test_pool_lifecycle() {
    ...
    let accounts = rpc.fetch_accounts(&pubkeys, None).await.unwrap();
    let cold: Vec<_> = accounts.into_iter().filter(|a| a.is_cold()).collect();

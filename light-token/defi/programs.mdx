...
cost savings). Your instructions stay the same. Inactive accounts auto-compress;
clients can load them back when needed.

| | Hot (active) | Cold (inactive) |
|---|---|---|
| Location | On-chain | Compressed |
...
| Area | Change |
|------|--------|
| State struct | Add a standard `compression_info: Option<CompressionInfo>` and derive `LightAccount` |
...
Audit scope is minimal as your program logic is mostly untouched. The rest is
macro-generated.
...
light-anchor-spl = { version = "0.31" }    # TokenInterface with light_token::ID
anchor-lang = "0.31"
```
...
pub struct PoolState {
    /// Required for auto-compression when cold.
...
To make it easy for clients to integrate with your program, ship an SDK crate implementing `LightProgramInterface`.

For a detailed example of how clients use this trait, check out the [Router Integration](./aggregators.mdx) page.
...
| Resource | Link |
|----------|------|
| SDK Reference | [csdk-anchor-full-derived-test-sdk](https://github.com/lightprotocol/light-protocol/tree/main/sdk-tests/csdk-anchor-full-derived-test-sdk) |
...
| Resource | Link |
|----------|------|
| Full test | [amm_test.rs](https://github.com/lightprotocol/light-protocol/blob/main/sdk-tests/csdk-anchor-full-derived-test/tests/amm_test.rs) |
...
| Stage | What happens |
|-------|--------------|
| Init | Pool created with rent-exemption sponsored by SDK |
| Hot | Any write bumps the virtual rent balance |
| Cold (eg., no writes for 24h) | Miner compresses to ledger |
| Load | User prepends load ix to transaction, pool back on-chain |


## Existing Programs

[Contact us](https://discord.com/invite/7cJ8BhAXhu) for migration support.
...
Questions or need hands-on support? [DM](https://t.me/swen_light) | [email](mailto:support@lightprotocol.com) | [Discord](https://discord.com/invite/7cJ8BhAXhu)
</Callout>

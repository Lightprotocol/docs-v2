...
<Accordion title="How does it prevent re-init attacks?"> When you create an
account, the SDK, in addition to paying the rent exemption cost, auto-provides a
proof that verifies that the account does not yet exist in the cold
address space. The SVM already does the same for the onchain address space. This
way both hot and cold address spaces are covered, preventing re-init attacks. </Accordion>

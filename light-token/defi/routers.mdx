...
The only addition: when a market has cold accounts, prepend load instructions before the swap.
...
You already stream account state (e.g. via [LaserStream](https://docs.helius.dev/laserstream/overview)).
Add a `cold_set: HashSet<Pubkey>` alongside your existing cache. When an account goes
cold (balance drops to zero in a `compress_and_close` transaction), add it to the set.
When an account comes back hot (account subscription delivers data), remove it.

For full streaming setup, see [Streaming Token Accounts](/light-token/data-streaming/for-streaming-tokens)
and [Streaming Mints](/light-token/data-streaming/for-streaming-mints).

If you don't stream and just want point queries, `get_account_interface` returns
both hot and cold account data transparently.
...
fetch their `ColdContext` and build load instructions.
...
Accounts go cold after extended inactivity. Their virtual rent balance falls
below a threshold and foresters compress them onto the Solana ledger.

They stay cold until a client loads them back in-flight via `create_load_instructions`.
...
<Accordion title="I don't stream. Can I still support cold markets?">

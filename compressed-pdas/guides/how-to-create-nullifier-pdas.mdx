...
Nullifier-program is a utility that lets you easily create rent-free nullifiers.
Prepend or append this instruction to your transaction to prevent your main
action from being executed more than once.
...
| **Cost** | ~15,000 lamports per nullifier (no rent) |
| **Source** | [github.com/Lightprotocol/nullifier-program](https://github.com/Lightprotocol/nullifier-program/) |
...
```toml
[dependencies]
light-nullifier-program = { git = "https://github.com/Lightprotocol/nullifier-program" }
light-client = "0.16.0"
```
...
```bash
npm install @lightprotocol/nullifier-program @lightprotocol/stateless.js
```
...
// Step 1: Fetch validity proof
let proof_result = fetch_proof(&mut rpc, &id).await?;

// Step 2: Build instruction (sync, no RPC)

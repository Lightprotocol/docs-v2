...
<Tip>
By default, we recommend to create cToken accounts with compressible extension to benefit from sponsored rent-exemption.
</Tip>


## Key Points

cTokens are Solana accounts that hold token balances like SPL token accounts for a specific mint. 
Each token account tracks:
...
1. Initialize cToken Account
2. **Set the initial lamports balance** to N epochs (must be at least 2 epochs)
...
3. **Set the top-up amount** for writes when the account balance falls below the set lamports balance.
...
When creating a compressible account, you configure `lamports_per_write` to automatically add rent on each write ("Top-Up").

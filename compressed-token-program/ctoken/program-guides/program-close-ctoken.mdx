...
2. cToken accounts can be closed
    ...
    * by the <Tooltip tip="The `compression_authority` is a PDA of the Light Registry program. It claims rent for the rent sponsor from compressible accounts that are compressed and closed.">`compression_authority`</Tooltip> when the account becomes compressible.
    The account is compressed and closed - it can be reinstated with the same state (decompressed).

...
2. `lamports_per_write` determines the amount a the transaction payer top-ups the account with rent.
    * This is set to 766 lamports by default.
    * Top-ups are made when the lamports balance falls below two epochs of rent.
3. The account is compressed after it is inactive for 2 epochs.
4. When the account is written to, it’s automatically decompressed.
    * The transaction payer tops up the account with 766 lamports.

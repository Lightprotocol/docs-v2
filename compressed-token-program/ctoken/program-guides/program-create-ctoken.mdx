...
2. cToken accounts are compressible, which means
    ...
    * At creation, the account creator pays <Tooltip tip="1 epoch ≈ 13,500 slots at 400ms per slot">24h / ~16 epochs</Tooltip> worth of <Tooltip tip="Rent for 260-byte cToken account per epoch: 360 bytes. Learn more in the [core concepts to compressible rent](/compressed-token-program/c-token-program#compressible-extension).">rent by default (~11,000 lamports)</Tooltip>. You can customize this amount.
...
3. To continously fund the account with rent, the transaction payer tops-up the lamports balance, when the account's lamports balance falls below two epochs of rent (766 lamports).
    * The top-up is configured to 766 lamports by default to cover rent for another ~3h / 2 epochs. You can customize this amount.

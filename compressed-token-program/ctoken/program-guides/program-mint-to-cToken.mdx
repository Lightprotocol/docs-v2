...
cTokens are minted via CPI to the Compressed Token Program and increase the supply of a cMint.
1. The destination cToken accounts must exist to receive the minted cTokens.
2. Only the mint authority can mint new cTokens.

<Tip>
* `MintToCToken` mints from a cMint to cToken Solana accounts. Use when you need Solana accounts for interoperability with DeFi, etc.

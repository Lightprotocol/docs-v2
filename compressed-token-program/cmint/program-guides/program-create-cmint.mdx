...
* The `mint_signer` keypair/PDA derives both the SPL mint address and the cMint's `compression_address`.
...
Your program's responsibility:
1. Deserialize the instruction data
2. Build the SDK structs (CreateCMintParams, SystemAccountInfos, CreateCMintAccountInfos)
3. Call .invoke() or .invoke_signed()
...
* Define the cMint struct to deserialize the instruction data correctly.
* The client passes a validity proof that proves the address of the cMint does not exist yet in the address tree. 
Your program validates this proof by passing it to the CPI. 
...
*  Both mint_signer and authority are PDAs.

<Tabs>
<Tab title="invoke (External Signer)">


// mint_signer is a keypair - user signs the transaction
...
<Tab title="invoke_signed (Two PDA signer)">

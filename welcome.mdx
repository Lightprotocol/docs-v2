...
<Steps>
  <Step title="State Compression">
    * Normally data is stored in millions of on-chain accounts * State
    compression stores account data on the{" "}
    ...
    . * Only a fingerprint is stored on-chain for cryptographic security - a hash
    of all compressed accounts. * This hash allows transactions to use the account
    data inside Solana's virtual machine as if it were stored on-chain.
  </Step>
  <Step title="ZK">
    * The protocol uses 128 byte zero-knowledge proofs (validity proofs) to
    verify the integrity of the compressed accounts. * By default, this is all
    done under the hood. You can fetch validity proofs from RPC providers that
    support ZK Compression.
  </Step>
</Steps>

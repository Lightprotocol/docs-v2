...
description: Create compressed token mint with metadata in TypeScript
allowed-tools: [Bash, Read, Glob, Grep, Task, mcp__deepwiki, mcp__zkcompression]
---

## Task: Create compressed token mint (TypeScript)
...
| Create Metadata | createTokenMetadata() | Configure token name, symbol, and URI |
| Create Mint (Action) | createMintInterface() | High-level action that creates mint and returns result |
| Create Mint (Instruction) | createMintInstruction() | Build instruction for manual transaction construction |
| Derive Address | findMintAddress() | Derive mint PDA from mint signer |
| Fetch Proof | getValidityProofV2() | Get proof that mint address does not exist yet |
